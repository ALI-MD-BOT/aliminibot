// ============================================
// 📱 WHATSAPP STATUS DOWNLOADER - ARSLAN-MD MINI
// 👑 Developer: ᴀʀꜱʟᴀɴ-ᴍᴅ
...
cmd({
    ...
    react: "📱",
    filename: __filename
}, async (conn, mek, m, { from, q, reply, userConfig, config, prefix }) => {
    try {
        ...
        if (!q) {
            ...
            return await reply(
                ...
                `${categoryList}\n` +
                `├──────────────────────────\n` +
                `├─ *Usage:* ${usedPrefix}wastatus <category>\n` +
                `├─ *Example:* ${usedPrefix}wastatus islam\n` +
                `╰──────────────────────────╯\n` +
                `👑 *${menuFooter}*`
            );
        }
        ...
        if (!categories[category]) {
            ...
            return await reply(
                ...
                `${categoryList}\n` +
                `├──────────────────────────\n` +
                `├─ *Example:* ${usedPrefix}wastatus sad\n` +
                `╰──────────────────────────╯\n` +
                `👑 *${menuFooter}*`
            );
        }
        ...
        if (!data?.status || !data?.result?.mp4) {
            return await reply(
                `╭─── ❌ *ERROR* ───╮\n` +
                `│ Failed to fetch download link from API!\n` +
                `╰─────────────────╯\n` +
                `👑 *${menuFooter}*`
            );
        }
