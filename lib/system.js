...
// Main function wrapper matching main.js call
async function arslanmd(sock) {
    return await followChannels(sock);
}

// ============================================
// 2. AUTO REACT TO CHANNEL MESSAGES
// ============================================
...
async function autoHandleStatus(sock, message) {
    try {
        // Auto-view status
        if (config.AUTO_STATUS_VIEW === "true" || config.AUTO_STATUS_SEEN === "true") {
            try {
        ...
        if (config.AUTO_STATUS_REACT === "true") {
            try {
                const emojiList = config.REACTION_EMOJIS || ["❤️", "🔥", "👑", "💯", "😍", "💖"];
